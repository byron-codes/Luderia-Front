...
export default class AccessoriesList extends Component {
  ...
  componentDidMount() {
    axios.get(`${baseURL}/accessories`).then(result => {
      this.setState({
          ...
          <tr key={item.id} data-cy={item.id}>
            ...
            <th className="d-flex justify-content-center">
              <Button
                ...
                className="mr-2"
                onClick={e => (window.location = `/admin/accessory/${item.id}`)}
              >
              ...
              <Button
                ...
                onClick={e => {
                  swal({
                    ...
                    axios
                      .put(
                        `${baseURL}/accessories/${item.id}/stock?quantity=${quantity}`
                      )
                      .then(result =>
                        swal(
                          "Sucesso",
  ...
  deleteRow(id) {
    axios.delete(`${baseURL}/accessories?id=${id}`).then(
      result => {
        swal(
          "Sucesso",
          "Cadastro de acessório deletado com sucesso",
          "success"
        ).then(result => window.location.reload());
      },
      error => {
